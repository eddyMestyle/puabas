...
const proxyUrl = "https://7d8c-49-49-62-225.ngrok-free.app"; // CORS Proxy URL
...
const App = () => {
  ...
  const storeUserProfile = async (profile) => {
    try {
      await axios.post(`${proxyUrl}/items/user`, {
        line_user_id: profile.userId, // ใช้ Line_user_ID เป็น User_ID
  ...
  const finishRandomize = async () => {
    ...
    try {
      await liff.sendMessages([
        { type: "text", text: `คุณได้รับโบนัสคะแนน: ${newScore} Points` },
      ]);
    } catch (err) {
  ...
  // ฟังก์ชันดึงข้อมูล LIFF Profile และบันทึกไปยัง Directus
  const fetchUserProfile = async () => {
    try {
      const profile = await liff.getProfile();
      setUserProfile(profile);

      // บันทึก User Profile ลงใน Directus ก่อนดึงสิทธิ์การสุ่ม
      await storeUserProfile(profile);

      // ดึงข้อมูลสิทธิ์การสุ่มหลังจากบันทึกโปรไฟล์ผู้ใช้สำเร็จ
    ...
    >
      <Card
        sx={{
          backgroundColor: colors.white,
          borderRadius: "30px",
          boxShadow: `20px 20px 60px ${colors.black}20, -20px -20px 60px ${colors.white}`,
          padding: "30px",
          width: "100%",
          maxWidth: "400px",
          textAlign: "center",
      ...
      >
        <NeumorphicBox sx={{ marginBottom: "30px" }}>
          {score !== null ? (
            <Box>
              <Typography
                sx={{
                  fontSize: isAnimating ? "64px" : "72px",
                  fontWeight: "bold",
        ...
        <CardContent>
          ...
          >
            จำนวนสิทธิ์ในการสุ่มรับโบนัส: {remainingChances}
          </Typography>
          <Typography
            sx={{
              fontSize: "18px",
          ...
          >
            คะแนนรวมที่สามารถขอรับโบนัสได้: {totalBonus} Points
          </Typography>
        </CardContent>
        <CardActions
          sx={{
        ...
        >
          <Button
            ...
            disabled={remainingChances === 0 || isAnimating}
            variant="contained"
            sx={{
              borderRadius: "50px",
              padding: "15px 20px",
              ...
              color: colors.white,
              boxShadow: `6px 6px 12px ${colors.black}20, -6px -6px 12px ${colors.white}`,
